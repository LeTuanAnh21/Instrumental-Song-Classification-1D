...
def load_data_from_folder(folder_path, len_audio = 220499):
    data = []
    label = []
    i_label = -1
...
if nkfold in [0,1]:
    ...
    if len(n_files)>0:
        ...
        from tensorflow.keras.callbacks import EarlyStopping
        model = Sequential()
        model.add(Conv1D(filters=64, kernel_size=3, input_shape=(args.lengthaudio,1)))
    ...
    # model.save(name_saved +'.h5', save_format='tf')
    # model_json = model.to_json()
    # with open(name_saved +".json", "w") as json_file:
